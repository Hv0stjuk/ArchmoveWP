...
let path = {
	...
	src: {
		...
		images: [src_folder + "/assets/img/**/*.{jpg,jpeg,png,svg,gif,ico,webp}", "!**/favicon.*"],
		fonts: src_folder + "/assets/fonts/*.ttf",
		json: src_folder + "/json/**/*.*"
	},
	watch: {
		html: src_folder + "/**/*.html",
		js: src_folder + "/assets/**/*.js",
		css: src_folder + "/assets/scss/**/*.scss",
		images: src_folder + "/assets/img/**/*.{jpg,jpeg,png,svg,gif,ico,webp}",
		json: src_folder + "/json/**/*.*"
	},
	clean: "./" + project_name + "/"
};
...
function fonts_otf() {
	return src('./' + src_folder + '/assets/fonts/*.otf')
		...
		.pipe(gulp.dest('./' + src_folder + '/assets/fonts/'));
}
function fonts() {
	src(path.src.fonts)
		.pipe(plumber())
...
function jsBuild() {
	let appPath = path.build.js + 'main.min.js';
	del(appPath);
	return src(path.src.js, {})
		.pipe(plumber())
		.pipe(fileinclude())
			...
			'append': {
				...
				'to': [
					...
					['image', '%TS%'],
					{
						...
						'value': '%DT%',
						'cover': 1,
						'files': ['main.min.js'] // Array [{String|Regex}] of explicit files to append to
					}
				]
			},
			'output': {
